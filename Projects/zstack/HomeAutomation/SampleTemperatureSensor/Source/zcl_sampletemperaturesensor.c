...
#define SAMPLETEMPERATURESENSOR_REPORT_INTERVAL   2500
...
//  zclSampleTemperatureSensor_DstAddr.endPoint = APP_ROUTER_ENDPOINT;
//  zclSampleTemperatureSensor_DstAddr.addr.shortAddr = NLME_GetCoordShortAddr();
  ...
  HalLcdWriteString( (char *)sDeviceName, HAL_LCD_LINE_3 );
	...
	osal_start_timerEx( zclSampleTemperatureSensor_TaskID, SAMPLETEMPERATURESENSOR_UART_REPLY_CMD_EVT, UART_PARSE_RX_PACKAGE_EVT_PERIOD );

	// Set timer for first Check System event
	osal_start_timerEx( zclSampleTemperatureSensor_TaskID, SAMPLETEMPERATURESENSOR_CHECK_SYSTEM_EVT, CHECK_SYSTEM_EVT_PERIOD );

	// Set timer for first Join Setup event
	if (NLME_GetShortAddr() != 0xFFFE)
	{
		osal_set_event( zclSampleTemperatureSensor_TaskID, SAMPLETEMPERATURESENSOR_JOIN_SETUP_EVT );
	}
...
uint16 zclSampleTemperatureSensor_event_loop( uint8 task_id, uint16 events )
{
      ...
      switch ( MSGpkt->hdr.event )
      {
        ...
        case ZDO_CB_MSG:
          zclSampleTemperatureSensor_ProcessZDOMsgs( (zdoIncomingMsg_t *)MSGpkt );
					UART_DebugPrint(HAL_UART_PORT_0, "ZDO_CB_MSG");
          break;
				#endif
        ...
        case ZCL_INCOMING_MSG:
          ...
          zclSampleTemperatureSensor_ProcessIncomingMsg( (zclIncomingMsg_t *)MSGpkt );
					UART_DebugPrint(HAL_UART_PORT_0, "ZCL_INCOMING_MSG");
          break;

        case KEY_CHANGE:
          zclSampleTemperatureSensor_HandleKeys( ((keyChange_t *)MSGpkt)->state, ((keyChange_t *)MSGpkt)->keys );
					UART_DebugPrint(HAL_UART_PORT_0, "KEY_CHANGE");
					break;
        ...
        case ZDO_STATE_CHANGE:
               ...
               (zclSampleTemperatureSensor_NwkState == DEV_END_DEVICE) )
          {
            ...
            zcl_EZModeAction( EZMODE_ACTION_NETWORK_STARTED, NULL );
						...
						UART_DebugPrint(HAL_UART_PORT_0, "ZDO_STATE_CHANGE");
          }
          break;
  ...
  if ( events & SAMPLETEMPERATURESENSOR_IDENTIFY_TIMEOUT_EVT )
  {
    ...
    zclSampleTemperatureSensor_ProcessIdentifyTimeChange();
		...
		UART_DebugPrint(HAL_UART_PORT_0, "SAMPLETEMPERATURESENSOR_IDENTIFY_TIMEOUT_EVT");
  ...
  if ( events & SAMPLETEMPERATURESENSOR_EZMODE_NEXTSTATE_EVT )
  {
    zcl_EZModeAction ( EZMODE_ACTION_PROCESS, NULL );   // going on to next state
		...
		UART_DebugPrint(HAL_UART_PORT_0, "SAMPLETEMPERATURESENSOR_EZMODE_NEXTSTATE_EVT");
  ...
  if ( events & SAMPLETEMPERATURESENSOR_EZMODE_TIMEOUT_EVT )
  {
    ...
    UART_DebugPrint(HAL_UART_PORT_0, "SAMPLETEMPERATURESENSOR_EZMODE_TIMEOUT_EVT");	
  ...
  if ( events & SAMPLETEMPERATURESENSOR_MAIN_SCREEN_EVT )
  {
    ...
    zclSampleTemperatureSensor_LcdDisplayUpdate();
		...
		UART_DebugPrint(HAL_UART_PORT_0, "SAMPLETEMPERATURESENSOR_MAIN_SCREEN_EVT");
    return ( events ^ SAMPLETEMPERATURESENSOR_MAIN_SCREEN_EVT );
  }

	/*--------------------------------------------------------------------------*/
	if ( events & SAMPLETEMPERATURESENSOR_SW1)
	{
      Toggle(Output_2);
	  zclSampleTemperatureSensor_SendState(Output_2);
	  return (events ^ SAMPLETEMPERATURESENSOR_SW1);
	}
	...
	if ( events & SAMPLETEMPERATURESENSOR_SW3 )
	{
      Toggle(Output_2);
	  Toggle(Output_3);
	  zclSampleTemperatureSensor_SendState(Output_3);	  
 ...
 ****************************************/
  ...
  {
  	//UART_DebugPrint(HAL_UART_PORT_0, "SAMPLETEMPERATURESENSOR_TEMP_SEND_EVT");
    zclSampleTemperatureSensor_SendTemp();			
	//UART_SendString(HAL_UART_PORT_0,"Teddy_sample");	
	...
	/*--------------------------------------------------------------------------*/
	if ( events & SAMPLETEMPERATURESENSOR_JOIN_SETUP_EVT )
  {
		if (FLAG_JOIN_CONFIRM == FALSE && FLAG_BINDING == FALSE)
		{
			...
			osal_start_timerEx( zclSampleTemperatureSensor_TaskID, SAMPLETEMPERATURESENSOR_JOIN_SETUP_EVT, 5000 );
		}
		...
		UART_DebugPrint(HAL_UART_PORT_0, "SAMPLETEMPERATURESENSOR_JOIN_SETUP_EVT");

    return ( events ^ SAMPLETEMPERATURESENSOR_JOIN_SETUP_EVT );
  }
	/*--------------------------------------------------------------------------*/
	if ( events & SAMPLETEMPERATURESENSOR_UART_REPLY_CMD_EVT )
  {
		// My apps
    ...
    return (events ^ SAMPLETEMPERATURESENSOR_UART_REPLY_CMD_EVT);
  }
	/*--------------------------------------------------------------------------*/
	if ( events & SAMPLETEMPERATURESENSOR_CHECK_SYSTEM_EVT )
  {
		// My apps
    ...
    return (events ^ SAMPLETEMPERATURESENSOR_CHECK_SYSTEM_EVT);
  }
...
static void zclSampleTemperatureSensor_HandleKeys( byte shift, byte keys )
{
	...
	{
       osal_set_event(zclSampleTemperatureSensor_TaskID,SAMPLETEMPERATURESENSOR_SW3);	
	}
	if ( keys & HAL_KEY_SW_7 ) //~ sw2 on board
	{
		
       osal_set_event(zclSampleTemperatureSensor_TaskID,SAMPLETEMPERATURESENSOR_SW2);
	}
	if ( keys & HAL_KEY_SW_6 ) //~ SW1 on board
	{
	osal_set_event(zclSampleTemperatureSensor_TaskID,SAMPLETEMPERATURESENSOR_SW1);
	}
...
static void zclSampleTemperatureSensor_SendTemp( void )
{
	// if (DHT11_GetValue() == 0)
	// {
		// zclSampleTemperatureSensor_MeasuredValue = BUILD_UINT16(69, 96);
	// }
	// else
	// {
		// zclSampleTemperatureSensor_MeasuredValue = DHT11_GetPackageValue();
	// }

	// //HalLedBlink ( HAL_LED_1, 1, 50, 500 );
	// UART_DebugPrintNum(HAL_UART_PORT_0, NLME_GetShortAddr());
	// UART_DebugPrint(HAL_UART_PORT_0, "");
	// UART_DebugPrintNum(HAL_UART_PORT_0, NLME_GetCoordShortAddr());
	// UART_DebugPrint(HAL_UART_PORT_0, "");
	// UART_DebugPrintNum(HAL_UART_PORT_0, zclSampleTemperatureSensor_MeasuredValue);
	
	// UART_SendNum(HAL_UART_PORT_0,zclSampleTemperatureSensor_MeasuredValue);
	// UART_DebugPrint(HAL_UART_PORT_0, "");
	uint8 state1;
	uint8 state2;
	uint16 pos1 = 21;
	...
	state1 = CheckOutputState(Output_2);
	state2 = CheckOutputState(Output_3);
	data1 = BUILD_UINT16(state1,pos1);
	data2 = BUILD_UINT16(state2,pos2);
  ...
  if ( pReportCmd != NULL )
  {
    ...
    pReportCmd->attrList[0].attrData = (void *)(&data1);

    pReportCmd->attrList[1].dataType = ZCL_DATATYPE_INT16;
    pReportCmd->attrList[1].attrData = (void *)(&data2);	
...
{ 
  ...
  pInReportCmd = (zclReportCmd_t *)pInMsg->attrCmd;
	...
	/*************************** THERMOSTAT DEMAND ******************************/
	if ( ( pInReportCmd->attrList[0].attrID == 0x0008 ) &&
       ( pInReportCmd->attrList[1].attrID == 0x0007 ) &&
       ( pInReportCmd->attrList[2].attrID == 0x0029	) )
	{
  	UART_ZCmdPrint(HAL_UART_PORT_0, "ATTRID_HVAC_THERMOSTAT");
	}
...
/****************************************************************************
****************************************************************************/
void zclSampleTemperatureSensor_BindingProcess(void)
{ 
	if ( ( giTemperatureSensorScreenMode == TEMPSENSE_MAINMODE ) ||
			( giTemperatureSensorScreenMode == TEMPSENSE_HELPMODE ) )
	{

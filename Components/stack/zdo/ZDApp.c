        ...
        if ( ( (pChosenNwk = ZDApp_NwkDescListProcessing()) != NULL ) && (zdoDiscCounter > NUM_DISC_ATTEMPTS) )
        {
          ...
          else if ( devStartMode == MODE_REJOIN )
          {
            ...
            if ( ZDApp_RestoreNwkKey( FALSE )== TRUE )
            {
              ...
              {
                ZDApp_ChangeState(DEV_NWK_SEC_REJOIN_ALL_CHANNEL);
              }
              else
              {
...
void ZDO_NetworkFormationConfirmCB( ZStatus_t Status )
{
  ...
  if ( Status == ZSUCCESS )
  {
    ...
    HalLedSet ( HAL_LED_3, HAL_LED_MODE_ON );
		UART_DebugPrint(HAL_UART_PORT_0, "Coordinator started");
		FLAG_JOIN_CONFIRM = TRUE;
  ...
  else
  {
    HalLedSet ( HAL_LED_3, HAL_LED_MODE_FLASH );  // Flash LED to show failure    
		UART_DebugPrint(HAL_UART_PORT_0, "NetworkFormationConfirm failure");
		FLAG_JOIN_CONFIRM = FALSE;
  }
#endif
...
void ZDO_StartRouterConfirmCB( ZStatus_t Status )
{
  ...
  if ( Status == ZSUCCESS )
  {
    ...
    HalLedSet ( HAL_LED_3, HAL_LED_MODE_ON );
		UART_DebugPrint(HAL_UART_PORT_0, "Router started");
		FLAG_JOIN_CONFIRM = TRUE;
  ...
  {
    HalLedSet( HAL_LED_3, HAL_LED_MODE_FLASH );  // Flash LED to show failure
    UART_DebugPrint(HAL_UART_PORT_0, "StartRouterConfirm failure");
		FLAG_JOIN_CONFIRM = FALSE;
  }
#endif
...
void ZDO_JoinConfirmCB( uint16 PanId, ZStatus_t Status )
{
  ...
  if ( Status == ZSUCCESS )
  {
    ...
    HalLedSet ( HAL_LED_3, HAL_LED_MODE_ON );
		UART_DebugPrint(HAL_UART_PORT_0, "JoinConfirm device joined");
		FLAG_JOIN_CONFIRM = TRUE;
...
#if defined(BLINK_LEDS)
    HalLedSet ( HAL_LED_3, HAL_LED_MODE_FLASH );  // Flash LED to show failure
    UART_DebugPrint(HAL_UART_PORT_0, "JoinConfirm failure");
		FLAG_JOIN_CONFIRM = FALSE;
#endif
  }
